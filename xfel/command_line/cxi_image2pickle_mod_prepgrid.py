...
def crop_image_pickle(data, beam_x_in, beam_y_in, clens, filename):
  """
  ...
  print "moving x,y", beam_x_in, beam_y_in
  ...
  beam_x = int(round(beam_x_in))
  beam_y = int(round(beam_y_in))
  width = data['SIZE1']
  height = data['SIZE2']
  pixels = data['DATA']
  ...
  for clen in clens:
    data["DISTANCE"] = clen
    tmppath = os.path.join("%.3f_%.3f_%.3f"%(beam_x_in,beam_y_in,clen), filename)
    os.mkdir(os.path.dirname(tmppath))
    print "writing", tmppath
    easy_pickle.dump(tmppath, data)
...
def run(argv=None):
  ...
  for imgpath in paths:
    ...
    if command_line.options.bkgpix:
      from cbflib_adaptbx import cbf_binary_adaptor
      print "Loading %s to mask" % command_line.options.bkgpix
      M = cbf_binary_adaptor(command_line.options.bkgpix)
      mask = M.uncompress_implementation("buffer_based").uncompress_data() < 0
      data["DATA"] = data["DATA"].set_selected(mask, -2)

    data['OSC_RANGE'] = 0 # YAM

    assert command_line.options.crop
    """
    for x in numpy.arange(command_line.options.beam_center_x-2, command_line.options.beam_center_x+2, step=0.5):
      for y in numpy.arange(command_line.options.beam_center_y-2, command_line.options.beam_center_y+2, step=0.5):
        tmpd = copy.deepcopy(data)
        tmpd["BEAM_CENTER_X"] = x * pixel_size
        tmpd["BEAM_CENTER_Y"] = y * pixel_size
        tmpd = crop_image_pickle(tmpd)
        for clen in numpy.arange(command_line.options.distance-.2, command_line.options.distance+.2, step=0.05):
          tmpd["DISTANCE"] = clen
          tmppath = os.path.join("%.3f_%.3f_%.3f"%(x,y,clen), os.path.basename(destpath))
          os.mkdir(os.path.dirname(tmppath))
          easy_pickle.dump(tmppath, tmpd)"""

    mp_params = []
    for x in numpy.arange(command_line.options.beam_center_x-2, command_line.options.beam_center_x+2, step=0.5):
      for y in numpy.arange(command_line.options.beam_center_y-2, command_line.options.beam_center_y+2, step=0.5):
        mp_params.append((x, y))

    clens = numpy.arange(command_line.options.distance-.2, command_line.options.distance+.2, step=0.05)
    easy_mp.pool_map(fixed_func=lambda x: crop_image_pickle(data, x[0], x[1], clens, os.path.basename(destpath)),
                     args=mp_params, processes=command_line.options.nproc)

    """for (x, y), tmpd in zip(mp_params, tmpds):
      for clen in numpy.arange(command_line.options.distance-.2, command_line.options.distance+.2, step=0.05):
        tmpd["DISTANCE"] = clen
        tmppath = os.path.join("%.3f_%.3f_%.3f"%(x,y,clen), os.path.basename(destpath))
        os.mkdir(os.path.dirname(tmppath))
